...
void newRecord( fstream &insertInFile ) // create record
{
    ...
    if ( client.getAccountNumber() == 0 )
    {
        ...
        cerr << "Account is not created!" << endl;
...
void printAllRecords(fstream &printFromFile)
 {
    cout << "Account" << setw(12) << "Last Name" << setw(17) << "First Name" << setw(11) << "Balance" << endl;
    for( int accountNumber = 1; accountNumber <= 100; accountNumber++ )
    {
        printFromFile.seekg( ( accountNumber - 1) * sizeof( ClientData ) ) ;

        ClientData client;
        printFromFile.read( reinterpret_cast< char * >( &client ), sizeof( ClientData ) );
 ...
 int getValidAccountNumber( fstream &getNumFromFile )
 {
    for( int accountNumber = 1; accountNumber <= 100; accountNumber++ )
    {
        getNumFromFile.seekg( ( accountNumber - 1) * sizeof( ClientData ) ) ;

        ClientData client;
        getNumFromFile.read( reinterpret_cast< char * >( &client ), sizeof( ClientData ) );
